...
use vesting_base::msg::{Cw20HookMsg, ExecuteMsg, ExecuteMsgManaged, QueryMsg, QueryMsgHistorical};
...
fn remove_vesting_accounts() {
    ...
    let user1_vesting_amount: Uint128 = app
        ...
        .query_wasm_smart(vesting_instance.clone(), &msg)
        .unwrap();
    assert_eq!(user1_vesting_amount.clone(), Uint128::new(400u128));

    // Check owner balance
    let bal = query_balance(&app.wrap(), &owner, VESTING_TOKEN)
        .unwrap()
        .u128();
    assert_eq!(bal, TOKEN_INITIAL_AMOUNT - 400u128);
    ...
    let bal = query_balance(&app.wrap(), &owner, VESTING_TOKEN)
        .unwrap()
        .u128();
    assert_eq!(bal, TOKEN_INITIAL_AMOUNT - 200u128);

    // Check that the user has the tokens that they claimed.
    let bal = query_balance(&app.wrap(), &user1, VESTING_TOKEN)
        .unwrap()
        .u128();
    assert_eq!(bal, 200u128);
    ...
    let vesting_state: VestingState = app
        .wrap()
        .query_wasm_smart(vesting_instance, &QueryMsg::VestingState {})
        .unwrap();
...
fn instantiate_vesting(app: &mut App, cw20_token_instance: &Addr) -> Addr {
    ...
    app.execute_contract(token_manager, vesting_instance.clone(), &msg, &[])
        .unwrap();
